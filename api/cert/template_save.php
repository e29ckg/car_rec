...
if(!isset($errorMSG))
{
	...
	$sql = "INSERT INTO project_template(id, project_id, template_name, size, orientation, template_url) 
			VALUE(:id, :project_id, :template_name, :size, :orientation, :template_url);";       
	$query = $conn->prepare($sql);
	$query->bindParam(':id',$template_id, PDO::PARAM_INT);
	$query->bindParam(':project_id',$id, PDO::PARAM_INT);
	...
	$query = $conn->prepare($sql);
	$query->bindParam(':text_id',$template_id, PDO::PARAM_INT);
